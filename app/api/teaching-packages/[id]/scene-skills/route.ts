/**
 * PUT /api/teaching-packages/[id]/scene-skills — policy-constrained Teaching
 * Skills mutation (Module 2 W14, plan §P Step 15 · FR-TS-039/040/070 ·
 * VAL-TS-016/023). Body:
 * `{ actorRef, assignments: [{ sceneId, teachingSkills: { primary?, supporting?, classification? } }] }`.
 *
 * Only draft|rejected GOVERNED versions accept Skill writes, and only within
 * the flow position's policy; content is preserved (no regeneration) and flow
 * identity is never touched. The stage guard and the write-boundary validator
 * both apply underneath putScene.
 */
...
export async function PUT(req: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  try {
    ...
    return NextResponse.json(
      { error: { code: 'INTERNAL_ERROR', message: 'failed to set scene teaching skills' } },
      { status: 500 },
    );
  }
}
